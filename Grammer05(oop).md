...
* **set메서드 : ** 매개변수에 저장된 값을 검사하여 조건에 맞는 값일 때만 멤버변수의 값을 변경하도록 작성.
 ...
 * 추상클래스(abstract클래스)를 상속하면 에러가 발생한다.????
    * AbstractSub클래스에서 abstract메소드를 갖고있는 AbstractSuper클래스를 상속하면서 AbstractSub클래스도 추상클래스로 변경.
    * [해결방법]
   1. 상위클래스로 사용될 목적으로 만들어진 클래스라면 클래스 선엄부에 abstract추가.
...
상위클래스 object 생성 -> Parent->child 객체 생성 

참조형 변수-> 레퍼런스 변수

메소드 - >생성되는 객체가 무엇이냐에 따라 실행 여부 결정
